#cd /d D:\enemy_999_pina
#py convertSSBP.py effect_immortal_object.ssbp > log.txt
...
ANIME_DATA = collections.OrderedDict([
    ...
    ('fps', 'SHORT'),
    ('labelIdx', 'SHORT'),
    #('canvasSizeX', 'SHORT'), #not been used
    #('canvasSizeY', 'SHORT'), #not been used
    ('DUMMY', 'SHORT')
])
...
def get_cells(data, index, num_cells):
    ...
    for n in range(num_cells):
        ...
        for key, dtype in CELL.items():
            ...
            #print('DEBUG INFO get_cells:  '+str(key)+': '+str(val))
...
def get_cell_map(data, index):
    ...
    for key, dtype in CELL_MAP.items():
        ...
        #print('DEBUG INFO get_cell_map:  '+str(key)+': '+str(val))
...
def get_pack_data(data, index, anime_list_size):
    ...
    for n in range(anime_list_size):
        ...
        for key, dtype in ANIME_PACK.items():
            ...
            #print('DEBUG INFO get_pack_data:  '+str(key)+': '+str(val))
    ...
    for n in range(num_parts):
        ...
        for key, dtype in PART.items():
            try:
                ...
                lump.update({key : val})
                #print('DEBUG INFO get_part_data_array:  '+str(key)+': '+str(val))
            except:
                print('DEBUG INFO get_part_data_array EXCEPTION:key='+str(key))
...
def get_anime_data_array(data, index, num_anime, num_parts):
    ...
    for n in range(num_anime):
        lump = {}
        #print('DEBUG INFO get_anime_data_array: n='+str(n)+' index='+str(index))
...
def get_initial_data_array(data, index, num_parts):
    ...
    for n in range(num_parts):
        lump = {}
        #print('DEBUG INFO get_initial_data_array: n='+str(n))
        ...
        for key, dtype in INITIAL_ANIMATION.items():
            ...
            #print('DEBUG INFO get_initial_data_array:  '+str(key)+': '+str(val))
...
def get_frame_data_index_array(data, index, end_frame):
    ...
    for n in range(end_frame):
        ...
        #print('DEBUG INFO get_frame_data_index_array:  '+str(val))
...
def get_frame_data_array(data, index_array, num_parts):
    ...
    for index in index_array:
        ...
        for n in range(num_parts):
            ...
            #print('DEBUG INFO get_frame_data_array: n='+str(n))
            ...
            for key, dtype in FRAME_DATA.items():
                try:
                    ...
                    #print('DEBUG INFO get_frame_data_array:  '+str(key)+': '+str(val))
                except:
